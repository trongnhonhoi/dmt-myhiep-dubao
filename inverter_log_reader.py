...
# Bảng tra cứu mã lỗi Huawei SUN2000 tiêu chuẩn (Kèm mô tả tiếng Việt & khuyến nghị O&M)
...
def export_inverter_log_to_excel(inv_meta: Dict[str, Any], df_alarms: pd.DataFrame, df_run_log: pd.DataFrame) -> bytes:
    """Xuất toàn bộ dữ liệu giải mã Logger Inverter sang tệp Excel chuyên nghiệp"""
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        if not df_alarms.empty:
            exp_alarm = df_alarms[[
                "STT", "Mã Lỗi", "Tên Sự Cố (Việt)", "Tên Sự Cố (Anh)", "Mức Độ",
